...
insert into ebisaan.products (name, sub_category_id, stock_number, image, discount_price, actual_price, currency_id)
select rp.name, sc.id, ebisaan.random(1, 1000), rp.image, regexp_replace(rp.discount_price, '[₹,]', '', 'g')::numeric, regexp_replace(rp.actual_price, '[₹,]', '', 'g')::numeric, c.id
from raw.products rp left join ebisaan.sub_categories sc on rp.sub_category = sc.name left join ebisaan.currencies c on substring(rp.actual_price from 1 for 1) = c.symbol
